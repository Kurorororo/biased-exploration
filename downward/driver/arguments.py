...
COMPONENTS_PLUS_OVERALL = ["translate", "search", "validate", "extract", "analyze", "draw", "overall"]
DEFAULT_SAS_FILE = "output.sas"
DEFAULT_EXTRACT_INPUT = "dumped_edges.tsv"
DEFAULT_EXTRACT_OUTPUT = "search_space.tsv"
DEFAULT_DRAW_OUTPUT = "heatmap.png"
...
def _split_planner_args(parser, args):
    ...
    for option in options:
        if option == "--translate-options":
            ...
            curr_options = args.search_options
        elif option == "--extract-options":
            curr_options = args.extract_options
        elif option == "--analyze-options":
            curr_options = args.analyze_options
        elif option == "--draw-options":
            curr_options = args.draw_options
        else:
...
def _set_components_and_inputs(parser, args):
    ...
    if args.extract:
        args.components.append("extract")
    if args.analyze:
        args.components.append("analyze")
    if args.draw:
        args.components.append("draw")
...
def parse_args():
    ...
    components.add_argument(
        ...
        help="run extractor component")
    components.add_argument(
        "--analyze", action="store_true",
        help="run analyzer component")
    components.add_argument(
        "--draw", action="store_true",
        help="run drawer component")
    ...
    driver_other.add_argument("--extract-input", metavar="FILE")
    driver_other.add_argument("--extract-output", metavar="FILE")
    driver_other.add_argument("--draw-output", metavar="FILE")
    ...
    if not args.extract_input:
        args.extract_input = DEFAULT_EXTRACT_INPUT
    if not args.extract_output:
        args.extract_output = DEFAULT_EXTRACT_OUTPUT
    if not args.draw_output:
        args.draw_output = DEFAULT_DRAW_OUTPUT
