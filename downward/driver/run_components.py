...
REL_ANALYZE_PATH = os.path.join("analyze", "analyze.py")
REL_DRAW_PATH = os.path.join("draw", "draw.py")
if os.name == "posix":
    REL_SEARCH_PATH = "downward"
    VALIDATE = "validate"
    REL_EXTRACT_PATH = "extract"
elif os.name == "nt":
    REL_SEARCH_PATH = "downward.exe"
    VALIDATE = "validate.exe"
    REL_EXTRACT_PATH = "extract.exe"
else:
...
def run_extract(args):
    ...
    time_limit = limits.get_time_limit(
        args.extract_time_limit, args.overall_time_limit)
    memory_limit = limits.get_memory_limit(
        args.extract_memory_limit, args.overall_memory_limit)
    executable = get_executable(args.build, REL_EXTRACT_PATH)

    try:
        call.check_call(
            "extractor",
            [executable, args.extract_input, args.extract_output] + args.extract_options,
            time_limit=time_limit,
            memory_limit=memory_limit)
    except subprocess.CalledProcessError as err:
        return (err.returncode, False)
...
def run_analyze(args):
    logging.info("Running analyzer.")
    time_limit = limits.get_time_limit(
        args.analyze_time_limit, args.overall_time_limit)
    memory_limit = limits.get_memory_limit(
        args.analyze_memory_limit, args.overall_memory_limit)
    analyzer = get_executable(args.build, REL_ANALYZE_PATH)
    assert sys.executable, "Path to interpreter could not be found"
    options = ["--input", args.extract_output] + args.analyze_options
    cmd = [sys.executable] + [analyzer] + options
...
def run_draw(args):
    logging.info("Running drawer.")
    time_limit = limits.get_time_limit(
        args.draw_time_limit, args.overall_time_limit)
    memory_limit = limits.get_memory_limit(
        args.draw_memory_limit, args.overall_memory_limit)
    drawer = get_executable(args.build, REL_DRAW_PATH)
    assert sys.executable, "Path to interpreter could not be found"
    options = ["--input", args.extract_output, "--output", args.draw_output] + args.draw_options
    cmd = [sys.executable] + [drawer] + options
