...
public:
    explicit LinearWeightedHeapOpenList(const Options &opts);
...
template<class Entry>
LinearWeightedHeapOpenList<Entry>::LinearWeightedHeapOpenList(const Options &opts)
...
template<class Entry>
bool LinearWeightedHeapOpenList<Entry>::empty() const {
    return size == 0;
}
...
static shared_ptr<OpenListFactory> _parse(OptionParser &parser) {
    ...
    parser.add_option<double>("alpha", "coefficent", "1.0");
    ...
    if (parser.dry_run())
        ...
        return make_shared<LinearWeightedHeapOpenListFactory>(opts);
}

static Plugin<OpenListFactory> _plugin("linear_weighted_heap", _parse);
}
