...
public:
    explicit LinearWeightedOpenList(const Options &opts);
...
template<class Entry>
LinearWeightedOpenList<Entry>::LinearWeightedOpenList(const Options &opts)
...
template<class Entry>
bool LinearWeightedOpenList<Entry>::empty() const {
    return size == 0;
}
...
static shared_ptr<OpenListFactory> _parse(OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return make_shared<LinearWeightedOpenListFactory>(opts);
}

static Plugin<OpenListFactory> _plugin("linear_weighted", _parse);
}
