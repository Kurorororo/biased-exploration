...
public:
    explicit LinearWeightedTypeBasedOpenList(const Options &opts);
...
template<class Entry>
LinearWeightedTypeBasedOpenList<Entry>::LinearWeightedTypeBasedOpenList(const Options &opts)
...
template<class Entry>
bool LinearWeightedTypeBasedOpenList<Entry>::empty() const {
...
static shared_ptr<OpenListFactory> _parse(OptionParser &parser) {
    parser.document_synopsis(
        "LinearWeightedType-based open list",
    ...
    parser.add_option<double>("alpha", "coefficient", "1.0");
    parser.add_option<double>("beta", "bias", "1.0");
    ...
    if (parser.dry_run())
        ...
        return make_shared<LinearWeightedTypeBasedOpenListFactory>(opts);
}

static Plugin<OpenListFactory> _plugin("linear_weighted_type_based", _parse);
}
