...
public:
    explicit SoftminHeapOpenList(const Options &opts);
...
template<class Entry>
SoftminHeapOpenList<Entry>::SoftminHeapOpenList(const Options &opts)
...
template<class Entry>
bool SoftminHeapOpenList<Entry>::empty() const {
    return size == 0;
}
...
static shared_ptr<OpenListFactory> _parse(OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return make_shared<SoftminHeapOpenListFactory>(opts);
}

static Plugin<OpenListFactory> _plugin("softmin_heap", _parse);
}
