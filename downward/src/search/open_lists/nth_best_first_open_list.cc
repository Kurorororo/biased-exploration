...
public:
    explicit NthBestFirstOpenList(const Options &opts);
...
template<class Entry>
NthBestFirstOpenList<Entry>::NthBestFirstOpenList(const Options &opts)
...
template<class Entry>
bool NthBestFirstOpenList<Entry>::empty() const {
    return size == 0;
}
...
static shared_ptr<OpenListFactory> _parse(OptionParser &parser) {
    ...
    parser.add_option<int>("n", "n", "2");

    Options opts = parser.parse();
    if (parser.dry_run())
        return nullptr;
    else
        return make_shared<NthBestFirstOpenListFactory>(opts);
}

static Plugin<OpenListFactory> _plugin("nth", _parse);
}
